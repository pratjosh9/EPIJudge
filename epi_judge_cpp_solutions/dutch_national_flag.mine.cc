...
void DutchFlagPartition(int pivot_index, vector<Color>* A_ptr) {
  vector<Color>& arr = *A_ptr;
  Color pivot = arr[pivot_index];
  int i = 0, j = arr.size()-1, currElem = 0;
  while (currElem <= j) {
    if (arr[currElem] < pivot){
      std::swap(arr[currElem++], arr[i++]);
    }else if (arr[currElem] > pivot){
      std::swap(arr[currElem], arr[j--]);
    }else currElem++;
  }
  return;
}
